...
# Use Windows Forms to bring up a file selection dialog for the user to select a video or audio file
Add-Type -AssemblyName System.Windows.Forms
$OpenFileDialog = New-Object System.Windows.Forms.OpenFileDialog
$OpenFileDialog.Title = "Select a video or audio file"
$OpenFileDialog.Filter = "Media Files|*.mp4;*.mkv;*.avi;*.mov;*.flv;*.wmv;*.mp3;*.wav;*.flac;*.aac"

if ($OpenFileDialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
    $INPUT_MEDIA = $OpenFileDialog.FileName
} else {
    Write-Host "No file selected, exiting."
    exit 1
}

# Determine if the selected file is an audio file
$audioExtensions = @(".mp3", ".wav", ".flac", ".aac")
$isAudioFile = $audioExtensions -contains [System.IO.Path]::GetExtension($INPUT_MEDIA).ToLower()

if ($isAudioFile) {
    $OUTPUT_AUDIO = $INPUT_MEDIA
    Write-Host "Selected file is an audio file: $OUTPUT_AUDIO"
} else {
    # Set the output file name (same name as input but with .wav extension)
    $OUTPUT_AUDIO = [System.IO.Path]::ChangeExtension($INPUT_MEDIA, ".wav")

    # Extract audio from the selected video and save it as .wav
    $ffmpegCommand = "ffmpeg -i `"$INPUT_MEDIA`" -vn -acodec libmp3lame -b:a 128k `"$OUTPUT_AUDIO`""
...
# Extract the base name of the INPUT_MEDIA file (without path and extension)
$BASE_NAME = [System.IO.Path]::GetFileNameWithoutExtension($INPUT_MEDIA)

# Save the response as an .srt file in the same folder as the audio file
$OUTPUT_SRT = [System.IO.Path]::ChangeExtension($INPUT_MEDIA, ".srt")
...
$OUTPUT_TXT = [System.IO.Path]::ChangeExtension($INPUT_MEDIA, ".txt")
...
$NEW_DIR = Join-Path -Path (Get-Item -Path $INPUT_MEDIA).DirectoryName -ChildPath $BASE_NAME
New-Item -ItemType Directory -Path $NEW_DIR -Force

# Move the original video, extracted audio, and SRT file to the new directory
Move-Item -Path $INPUT_MEDIA, $OUTPUT_AUDIO, $OUTPUT_SRT, $OUTPUT_TXT -Destination $NEW_DIR
